...
from applic.exceptions import USER_NOT_FOUND
...
class TestProfile:
    def test_get_profile_not_loggedin( self, testapp ):
        ...
        resp = testapp.get( url_for( 'profiles.get_profile',
                                     username = 'foobar' ) )
    ...
    def test_follow_user( self, testapp, user ):
        ...
        resp = testapp.post( url_for( 'profiles.follow_user',
    ...
    def test_unfollow_user( self, testapp, user ):
        ...
        resp = _register_user( testapp )
        token = str(resp.json[ 'user' ][ 'token' ] )
        resp = testapp.delete( url_for( 'profiles.unfollow_user',
                                        username = user.username ),
                               headers = {
                                   'Authorization': 'Token {}'.format( token )
                               } )
