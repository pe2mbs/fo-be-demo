...
@cross_origin()
def apiPagedContacts():   
    global contacts
    offset = request.args.get( 'offset' )
    count = request.args.get( 'count' )
    print( 'apiContacts( %s, %s )' % ( offset, count ) )
    if offset is None or count is None:
        abort( 428 )

    pagerInfo = getPager( len( contacts ), int( offset ), int( count ) )
    pagedItems = contacts[ pagerInfo[ 'startIndex' ]: 
...
@app.route( "/api/contact/<int:id>", methods = [ 'GET', 'PUT', 'PATCH', 'DELETE' ] )
@cross_origin()
def apiContact( id ):
    record = {}
    print( "apiContact", id )
...
def testPager():
    ...
    print( json.dumps( { 'pagedItems': pagedItems,
                         'pagerInfo': pagerInfo }, 
                         indent= 4 ) ) 
    return 
